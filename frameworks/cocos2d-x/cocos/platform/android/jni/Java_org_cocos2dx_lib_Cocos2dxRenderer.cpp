...
extern "C" {
    ...
    JNIEXPORT jstring JNICALL Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeGetContentText() {
        JNIEnv * env = 0;
        //__android_log_print(ANDROID_LOG_DEBUG,"ANDROID","Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeGetContentText");
        if (JniHelper::getJavaVM()->GetEnv((void**)&env, JNI_VERSION_1_4) != JNI_OK || ! env) {
            return 0;
        }
        std::string pszText = cocos2d::IMEDispatcher::sharedDispatcher()->getContentText();
        //__android_log_print(ANDROID_LOG_DEBUG,"ANDROID","pszText %s", pszText.c_str());
